...
WeightFileName = ["light.hdf5","bw_4015imgs_weights.hdf5","bw_2510imgs_weights.hdf5","./bw_weight.hdf5","./final_c_weights.hdf5","./semiVgg_1_weights.hdf5","/new_wt_dropout20.hdf5","./weights-CNN-gesture_skinmask.hdf5"]
...
output = ["BACK", "LEFT", "NEUTRAL", "NOTHING", "RIGHT", "SELECT"]
...
def getBestGuess():
    ...
    maxValue = -1
    maxKey = ""
    for itemName in jsonarray:
        if maxValue < jsonarray[itemName]:
            maxValue = jsonarray[itemName]
            maxKey = itemName
    return maxKey
...
def initializers():
    ...
    immatrix = np.array([cv2.resize(np.array(Image.open(path2+ '/' + images).convert('L')), dsize=(img_rows, img_cols), interpolation=cv2.INTER_CUBIC).flatten()
