...
#%%
def Main():
    global guessGesture, visualize, mod, binaryMode, bkgrndSubMode, mask, takebkgrndSubMask, x0, y0, width, height, saveImg, gestname, path, uiOn
    quietMode = False
    ...
    cv2.namedWindow('Original', cv2.WINDOW_AUTOSIZE) #cv2.WINDOW_NORMAL)
    ...
    while(True):
        ...
        frame = cv2.resize(frame, (1200,1024))
        centerX, topY = 200,200
                      
        if ret == True:
            if bkgrndSubMode == True:
                roi, centerX, topY = bkgrndSubMask(frame, x0, y0, width, height, framecount, plot)
                # cv2.circle(frame, (centerX, topY), 3, (200,0,0))
        ...
        if (displayOptions):
            ...
            cv2.putText(frame,'u - To start the UI demo',(fx,fy + 7*fh), font, size,(0,255,0),1,1)
            cv2.putText(frame,'ESC - Exit',(fx,fy + 8*fh), font, size,(0,255,0),1,1)

        if (uiOn):
            gestureGuess = myNN.getBestGuess()
            uiDemo.newGuess(gestureGuess)
            uiText = uiDemo.getCurrentText()
            fontWidth = cv2.getTextSize(uiText, font, 2, 2)[0][0] #weird return format, but okay
            # print(fontWidth)
            if gestureGuess is not "NOTHING" and gestureGuess is not '':
                cv2.putText(frame,uiText,(670 + centerX - int(fontWidth/2),450 + topY), font, 2,(0,255,0),2,1)
